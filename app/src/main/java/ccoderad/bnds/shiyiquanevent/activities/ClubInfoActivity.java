...
public class ClubInfoActivity extends AppCompatActivity {
    ...
    private void handleRawData(String response) {
        try {
            ...
            if(clubData.has(ClubInfoConstants.EVENT_LIST_TAG)){
                eventData = clubData.getJSONArray(ClubInfoConstants.EVENT_LIST_TAG);
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mViewEvent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                it.putExtra("clubSimpName", clubUrl);
                startActivity(it);
            }
        });

        mPresentationBanner.setDelegate(new BGABanner.Delegate<CardView, PresentationModel>() {

            @Override
            public void onBannerItemClick(BGABanner banner, CardView itemView, PresentationModel model, int position) {
                View alertView = ViewTools.Inflate(ClubInfoActivity.this
                        , R.layout.alert_presentation, null);
                TextView title = (TextView) alertView.findViewById(R.id.alert_presentation_title);
                SimpleDraweeView img = (SimpleDraweeView) alertView.findViewById(R.id.alert_presentation_img);
                title.setText(model.title);
                img.setImageURI(Uri.parse(model.imageUrl));
                new AlertDialog.Builder(ClubInfoActivity.this)
                        .setView(alertView).setPositiveButton("关闭", new DialogInterface.OnClickListener() {
                    @Override
